      ...
      <div v-for="order in orders" :key="order.order_id" class="order-card">
        ...
        <div class="order-footer">
          ...
          <div class="order-actions">
            <el-button v-if="order.status === 0" type="primary" @click="handlePay(order.order_id)">
              立即支付
            </el-button>
            <el-button v-if="order.status === 0" @click="handleCancel(order.order_id)">
              取消订单
            </el-button>
            <el-button v-if="order.status === 2" type="primary" @click="handleReceive(order.order_id)">
              确认收货
            </el-button>
            <el-button v-if="order.status === 3" @click="goToReview(order.order_id)">
              去评价
            </el-button>
            <el-button @click="goToDetail(order.order_id)">查看详情</el-button>
          </div>
        </div>
      </div>
    </div>
...
const handleReceive = async (orderId) => {
  try {
    ...
    await api.order.receive(orderId)
...
const goToReview = (orderId) => {
  router.push(`/order/${orderId}?action=review`)
}
